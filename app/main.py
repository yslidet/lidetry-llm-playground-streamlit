...
def generate_outputs(selected_models, user_input):
    ...
    threads = [
        threading.Thread(
            target=model_output, 
            args=(model, user_input, output_queue)
        )
    ...
    while any(thread.is_alive() for thread in threads):
        try:
            ...
            # processed_output = outputs[name]
            placeholders[name].markdown(
                f'<div class="output-box">{processed_output}</div>',
...
def run_app():
    st.set_page_config(
        layout="wide", 
        page_title = "Multi-model App - LLM Playground"
    )
    # Apply CSS
